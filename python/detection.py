...
def analyze_single_image(image_path, drawing_type):
    ...
    probability_percent = round(consensus_confidence * 100)

    avg_accuracy = 0
    if "rf" in metrics and "xgb" in metrics:
        avg_accuracy = round((metrics["rf"]["accuracy"] + metrics["xgb"]["accuracy"]) / 2 * 100)
